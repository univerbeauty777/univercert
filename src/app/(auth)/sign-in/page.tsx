...
export default function SignInPage() {
  ...
  const handleMicrosoft = async () => {
    setLoading(true);
    try {
      await (signIn as any).social({ provider: 'microsoft', callbackURL: '/dashboard' });
    } catch {
      setError('Microsoft sign-in não disponível no momento');
      setLoading(false);
    }
  };
    ...
    <main className="min-h-screen bg-mesh flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="card-glass w-full max-w-md relative animate-scale-in shadow-card-lift p-8">
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          <button onClick={handleGoogle} disabled={loading} className="btn-secondary justify-center">
          ...
          <button onClick={handleMicrosoft} disabled={loading} className="btn-secondary justify-center">
